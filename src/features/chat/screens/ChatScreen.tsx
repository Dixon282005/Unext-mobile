...
// DATOS MOCK - IDs coinciden con nombres de usuarios
const CHATS: Chat[] = [
  {
    id: "maria-rodriguez",
    name: "Maria Rodriguez",
    message: "¡Gracias! Nos vemos mañana...",
    time: "10:30 AM",
    unread: 2,
  ...
  },
  {
    id: "elon-musk",
    name: "Elon Musk",
    message: "Compramos Unext por $1B? 🚀",
    time: "Lun",
    unread: 1,
...
export function ChatListScreen() {
  ...
  const handleChatPress = (chatId: string) => {
    // Navegar a la conversación individual usando el ID
    ...
    <Screen safeArea className="bg-white">
      ...
      <View className="px-4 py-3 border-b border-slate-100 flex-row justify-between items-center">
        <View className="flex-row items-center">
          ...
          <ThemedText variant="h2" className="text-2xl font-bold">
            Chats
          </ThemedText>
        </View>
      ...
      <FlatList
        data={CHATS}
        renderItem={({ item }) => (
          <ChatRow chat={item} onPress={() => handleChatPress(item.id)} />
        )}
